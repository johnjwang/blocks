...
static comms_status_t publish(comms_t *comms, uint32_t tx_origin_num, uint8_t data)
{
    if(!comms_cfuncs->push_back(comms->buf_tx[tx_origin_num], &data))
        return COMMS_STATUS_BUFFER_FULL;

    if(comms_cfuncs->is_full(comms->buf_tx[tx_origin_num]))
...
#define PUBLISH_CHECK_FULL_BUFFER(C,N,D,S)\
    do { \
        ...
        { \
            ...
            { \
                comms_cfuncs->pop_back(comms->buf_tx[tx_origin_num]); \
                if(++num_popped == S) \
                    break; \
            } \
            return COMMS_STATUS_BUFFER_FULL; \
...
*/
    ...
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, START_BYTE1, num_so_far);

    fletcher_checksum_add_byte_tx(comms, tx_origin_num, START_BYTE2);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, START_BYTE2, num_so_far);

    uint8_t id1 = (id & 0xff00) >> 8;
    fletcher_checksum_add_byte_tx(comms, tx_origin_num, id1);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, id1, num_so_far);

    uint8_t id2 = id & 0x00ff;
    fletcher_checksum_add_byte_tx(comms, tx_origin_num, id2);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, id2, num_so_far);

    fletcher_checksum_add_byte_tx(comms, tx_origin_num, channel);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, channel, num_so_far);

    uint8_t len1 = (msg_len & 0xff00) >> 8;
    fletcher_checksum_add_byte_tx(comms, tx_origin_num, len1);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, len1, num_so_far);

    uint8_t len2 = msg_len & 0x00ff;
    fletcher_checksum_add_byte_tx(comms, tx_origin_num, len2);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, len2, num_so_far);

    uint8_t i;
    for(i = 0; i< msg_len; ++i)
    {
        fletcher_checksum_add_byte_tx(comms, tx_origin_num, msg[i]);
        PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num, msg[i], num_so_far);
    }

    fletcher_checksum_calculate_tx(comms, tx_origin_num);

    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num,
                              comms->checksum_tx1[tx_origin_num], num_so_far);
    PUBLISH_CHECK_FULL_BUFFER(comms, tx_origin_num,
                              comms->checksum_tx2[tx_origin_num], num_so_far);

    comms_status_t ret = publish_flush(comms, tx_origin_num);

    if((comms->curr_tx_orig_num == tx_origin_num) && (ret == COMMS_STATUS_DONE))
